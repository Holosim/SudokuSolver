...
## Also: watch for a first-attempt Edit/Write landing on the wrong branch

While fixing this, the very first draft of this ledger update was committed
and pushed straight to `main` — wrong, because it was a *first-pass*
`status:ready-for-rtvm-update` (Approved → In Test, no SHA yet), which
per [[rtvm-conventions]]'s "fast-path RTVM-update commit lands on the
feature branch, not main" rule belongs on `issue-17`, merged into trunk by
CI/CD together with the product diff. Caught it before handing off, fixed
with `git revert` on `main` (non-destructive, keeps history honest) followed
by redoing the same edits on a freshly `git fetch --unshallow`'d,
`origin/main`-merged `issue-17` checkout. Worth the reminder: check whether
you're writing the *first* promotion (branch) or *recording a SHA*
(main) before the first `git commit`, not after a push already happened —
reverting is fine but redoing full doc edits twice is exactly the kind of
cost that makes checking first cheaper.

See [[verification-platform-trap]], [[pipeline-label-traps]], [[rtvm-conventions]].
